...
public class StateMachine<T> {
    ...
    public StateMachine(String name) {
        ...
        // Set up the logger
        logger = LoggerFactory.getLogger(String.format("StateMachine_%s", name));
     ...
     */
    public void addState(T key, Function<StateMetadata<T>, T> action) {
        ...
        logger.debug(String.format("Added state: %s", key.toString()));
     ...
     * @param action Action to be run
     */
    public void setDefaultState(T key, Function<StateMetadata<T>, T> action) {
        ...
        // Set the default
        defaultStateKey = key;
        logger.debug(String.format("Set state %s as default", key.toString()));
    ...
    public void removeState(T key) {
        ...
        logger.debug(String.format("Removed state: %s", key.toString()));
    ...
    public void update() {
        ...
        T nextKey = state.call(isNew, lastStateKey);

        // If the next key is a new key, switch to it
        if (!nextKey.equals(desiredStateKey)) {
            setState(nextKey);
        }
    }
    ...
    public void setState(T key) {
        logger.debug("Switching to state: %s", key);
        desiredStateKey = key;
    }
